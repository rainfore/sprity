...
var getTemplate = function (opt) {
  var templatePath = path.resolve(opt.template);
...
var createStyle = function (opt) {
  return function (cb) {
    ...
    if (opt.style) {
      if (opt.template) {
        style = getTemplate(opt).then(Promise.method(function (source) {
          Handlebars.registerHelper('baseDim', function (size) {
            return Math.round(size * opt.ratio);
          });
          var template = Handlebars.compile(source);

          return template({
            layouts: opt._layouts
          });
        }));
      }
      else {
        style = prepareProcessor(opt)
          ...
          .then(function () {
            return processor.process(opt._layouts, opt, Handlebars);
          })
          .then(beautify(opt));
      }
...
module.exports = function (opt) {
  ...
  if (opt.dimension) {
    ...
    opt.ratio = opt.dimension[opt.dimension.length - 1].ratio / opt.dimension[0].ratio;
  }
  else {
    opt.ratio = 1;
  }
  return through2.obj(function (layout, enc, cb) {
    if (layout instanceof Error) {
      cb(layout);
    }
    else {
      if (layout) {
        opt._layouts.push(layout);
        cb(null, layout);
      }
      else {
        cb();
